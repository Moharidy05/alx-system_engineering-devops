...
Added the SSH public key to my server so as to connect using the ubuntu user.

connected to use ubuntu, using:
ssh -v ubuntu@98.98.98.98
ssh -v -F 2-ssh_config ubuntu@IP address
...
* SSH client configuration,configured to refuse to authenticate using a password
